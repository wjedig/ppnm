...
public static class Test{
	public static void testA(bool writeFit, int n = 10){
		var fs = new Func<double,double>[] { z => 1.0, z => z, z => z*z };
	...
	public static (vector, matrix) fitDecay(vector xs, vector ys, vector dys, bool writeFit){
		...
		if(writeFit){
			vector errs = new vector(Sqrt(S[0,0]),Sqrt(S[1,1]));
			WriteLine($"a = {c[0]}");
			WriteLine($"b = {c[1]}");
			WriteLine($"a_err = {errs[0]}");
			WriteLine($"b_err = {errs[1]}");
		}
		return (c,S);
...
class main{
	public static void Main(string[] args){
		...
		if(infile!=null){
			...
			(vector c, matrix S) = Test.fitDecay(xs,ys,dys,writeFit);
			if(writeCov){
				...
				double err = Sqrt(S[1][1]);
				WriteLine($"{Log(2)/c[1]} +/- {err*Log(2)/(Pow(c[1],2))} days");
				WriteLine("Compared to the theoretical value of 3.6 days");
				WriteLine("Our value does not fit this theoretical value within the uncertainties.");
			}
			//xs.print("xs = ");
			//ys.print("ys = ");
			//dys.print("dys = ");
		...
		foreach(string run in args){
			if(run=="-test"){Test.testA(writeFit);}
		}
	}
}
